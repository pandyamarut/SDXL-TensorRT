...
if __name__ == "__main__":
    ...
    if args.use_cuda_graph and (not args.build_static_batch or args.build_dynamic_shape):
        raise ValueError(
            f"Using CUDA graph requires static dimensions. Enable `--build-static-batch` and do not specify `--build-dynamic-shape`")
    ...
    demo_base = init_pipeline(Txt2ImgXLPipeline, False,
                              args.onnx_base_dir, args.engine_base_dir)
    demo_refiner = init_pipeline(
        Img2ImgXLPipeline, True, args.onnx_refiner_dir, args.engine_refiner_dir)
